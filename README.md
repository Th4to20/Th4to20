 👋 Hi, I’m Thato Goqo

   🎓 3rd-year Information Technology student at Belgium Campus iTversity
   💼Seeking internship/graduate opportunities in software Engineering
   🌱 Passionate about building responsive, multi-page web experiences and clean UI
   🚀 Skills & Expertise
Languages & Core Web: C#, Python, HTML5, CSS3, JavaScript (ES6+)
Frameworks & Tech:.NET 8, Three.js, gRPC, JSON-LD structured data
Systems & Hardware: Distributed Systems, Raspberry Pi Clusters, Hardware Troubleshooting

 📂 Featured Projects

TK Digital Studio (Freelance Practice Builds)
Independently designed and built responsive, multi-page demo websites for real businesses across the medical, hospitality, construction, and beauty industries to sharpen my skills and grow my portfolio. 

Live Demos:
    *   [Dr Lecheko GP Practice](https://drlecheko.netlify.app)
    *   [Madi Health Medical Centre](https://madihealthmedicalcentre.netlify.app)
    *   [Julia Mathabathe (Clinical Psychologist)](https://juliamathabathe.netlify.app)
    *   [Wonderboom Car Service](https://wonderboomcarservice.netlify.app)
    *   [TK Digital Studio](https://tkdigitalstudio.netlify.app)
   Key Features: Implemented real-time open/closed indicators, WhatsApp integration, JSON-LD structured data for SEO, dark mode, and Three.js 3D effects.
   Tech Stack:** HTML, CSS, JavaScript, JSON-LD, Three.js
  (Note: These are self-initiated practice builds hosted as live demos, not commissioned production sites run by these businesses.)*

Academic Projects & Collaborations
[PRJ381 Agent-Based Modelling](#): A C#/.NET 8 framework with Monte Carlo batch experimentation, built for distributed execution across a Raspberry Pi cluster via gRPC.
[TrackB Inventory Issuance App](https://github.com/Druxler/TrackB-Inventory-Issuance-App):Collaborative university cleaning inventory & issuance system developed with a team partner.

📄[Click here to view & download my CV](https://th4to20.github.io/Thato-Goqo-portfolio-website/assets/Thato_Goqo_CV.pdf)

